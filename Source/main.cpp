...
void OnMouseWheel(WPARAM wParam,LPARAM lParam) {
	signed short scrollamount = HIWORD(wParam);
	int multiplier = 100;
	if(scrollamount > 0) {
		camera->Move(multiplier*camera->dir);
	} else {
		camera->Move(-multiplier*camera->dir);
	}
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	wc.hIcon         = LoadIcon(hInstance,"A"); // load project icon
	wc.hIconSm       = LoadIcon(hInstance,"A"); // load project icon
